...
var filterList=['emerg','alert','crit','err','warning','notice','info'];function filterSeverity(selectObject){var table=document.getElementById("syslogTable");for(const severity of filterList){table.classList.toggle("filter_"+severity,selectObject.value==severity)}}
