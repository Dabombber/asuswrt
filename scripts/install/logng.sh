...
web_mount() {
	...
	if [ -n "$TIMESTAMP" ]; then
		...
		sed -i -f - "$SYSLOG_WWW" <<-'EOSCRIPT'
			...
			s/Auto refresh<\/div>/<label for="auto_refresh">Auto refresh<\/label>\n<input type="checkbox" id="facility" onchange="document\.getElementById('syslogTable')\.classList\.toggle(this\.id, this\.checked);"><label for="facility">Facility<\/label>\n<input type="checkbox" id="hostname" onchange="document\.getElementById('syslogTable')\.classList\.toggle(this\.id, this\.checked);"><label for="hostname">Hostname<\/label>\n<div id="severityContainer">\n<select id="severity" class="input_option" onchange="filterSeverity(this)">\n<option value="alert" <% nvram_match("log_level", "1", "selected"); %>>alert<\/option>\n<option value="crit" <% nvram_match("log_level", "2", "selected"); %>>critical<\/option>\n<option value="err" <% nvram_match("log_level", "3", "selected"); %>>error<\/option>\n<option value="warning" <% nvram_match("log_level", "4", "selected"); %>>warning<\/option>\n<option value="notice" <% nvram_match("log_level", "5", "selected"); %>>notice<\/option>\n<option value="info" <% nvram_match("log_level", "6", "selected"); %>>info<\/option>\n<option value="debug" <% nvram_match("log_level", "7", "selected"); %>>debug<\/option>\n<option value="all" <% nvram_match("log_level", "8", "selected"); %>>all<\/option>\n<\/select>\n<label for="severity">or more urgent<\/label>\n<\/div>\n<\/div>/
			/showDST();/a\
			initSeverity();
...
return io==-1?-1:io+string.length};var lastLine="";var severityList=['emerg','alert','crit','err','warning','notice','info','debug'];var syslogWorker=new Worker("/user/logng_worker.js");syslogWorker.onmessage=function(e){if(!e.data.idx)return;var row=document.getElementById("syslogTable").rows[e.data.idx];var cell=row.insertCell(-1);if(e.data.facility){cell.innerText=e.data.facility;if(e.data.severity)cell.setAttribute("title",e.data.severity)}
...
function filterSeverity(selectObject){var table=document.getElementById("syslogTable");for(const severity of severityList){table.classList.toggle("filter_"+severity,selectObject.value==severity)}}
function initSeverity(){if(0<=<%nvram_get("log_level");%>&&<%nvram_get("log_level");%><=7){document.getElementById("syslogTable").classList.add("filter_"+severityList[<%nvram_get("log_level");%>])}}
EOF
