...
/*
 *  Main_LogStatus_Content.asp specific
 */
...
String.prototype.lastIndexEnd = function(string) {
  ...
  return (string.length == 0 || io == -1) ? -1 : io + string.length;
};

var lastLine = "";
var syslogParser = new LoggyParser();
function append_to_log(lines, log_level) {
  lines.substring(lines.lastIndexEnd(lastLine)).split("\n").filter(i => i).forEach(line => syslogParser.parse(line, (msg) => {
    lastLine = "\n" + msg.originalMessage + "\n";
    if(msg.severityID == null || msg.severityID < log_level){
      document.getElementById("textarea").innerHTML += msg.time.to8601String() + " " + msg.header + msg.message + "\n";
    }
  }));
}
